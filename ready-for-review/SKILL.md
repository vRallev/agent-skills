...
11. After the push succeeds, use `$create-or-update-pr` to update the existing PR from the finalized single commit. This step is required on every run, even when the PR already has a description or no squash was needed:
...
12. Fetch all pull-request review threads with a thread-aware GitHub connector tool. Resolve every unresolved inline review conversation after the push. Do not post comments or replies unless the user asks for them. When asked, use `$github-comments` for every comment or review write. Note that top-level PR comments do not have a resolvable conversation state.
