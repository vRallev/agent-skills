...
12. Fetch all pull-request review threads with a thread-aware GitHub connector tool. Resolve every unresolved inline review conversation after the push. Do not post comments or replies unless the user asks for them. When the user asks for them, every comment posted on GitHub—including a top-level issue or pull-request comment, pull-request review body, new inline review comment, or reply—must start exactly with `**Ralf-AI:**`. Do not add this prefix to GitHub titles or pull-request descriptions. Note that top-level PR comments do not have a resolvable conversation state.
